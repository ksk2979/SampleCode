...
public class GameManager : Singleton<GameManager>
{
    ...
    public bool _indunStart = false;
    public bool _indunClear = false;
    public int[] _indunBoss = { 0, 0, 0, 0 };
    #endregion
    ...
    private void Update()
    {
        ...
        if (Input.GetKey(KeyCode.P) && Input.GetKey(KeyCode.O) && Input.GetKeyDown(KeyCode.A))
        {
            if (cSceneManager.GetInstance.GetSceneName("Lobby"))
            {
                UserData.GetInstance.AddCurrency(EPropertyType.INDUNBOSSTICKET, 1);
                LobbyUIManager.GetInstance.GetPopup<IndunPopup>(PopupType.INDUNPOPUP).IndunBossTicketCountUpdate();
            }
        }
        if (Input.GetKey(KeyCode.P) && Input.GetKey(KeyCode.O) && Input.GetKeyDown(KeyCode.D))
        {
            if (cSceneManager.GetInstance.GetSceneName("Lobby"))
            {
                LobbyUIManager.GetInstance.OnTouchAddDia();
            }
        }
        if (Input.GetKey(KeyCode.P) && Input.GetKey(KeyCode.O) && Input.GetKeyDown(KeyCode.M))
        {
            if (cSceneManager.GetInstance.GetSceneName("Lobby"))
            {
                LobbyUIManager.GetInstance.OnTouchAddAllMaterial();
            }
        }
    }
    ...
    public void IndunBossSetting(int id, int levelArr, int reward, int rewardCount)
    {
        _indunBoss[(int)EIndunType.BOSSID] = id;
        _indunBoss[(int)EIndunType.BOSSLEVELARR] = levelArr;
        _indunBoss[(int)EIndunType.BOSSREWARD] = reward;
        _indunBoss[(int)EIndunType.BOSSREWARDCOUNT] = rewardCount;
    }
    ...
    public bool ADCheck { get { return _adCheck; } set { _adCheck = value; } }

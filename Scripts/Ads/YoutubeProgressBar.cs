...
public class YoutubeProgressBar : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IPointerDownHandler
{
    /// <summary>
    ...
    void Awake()
    {
        ...
        m_PlaybackProgress.enabled = false;
...
/*    void OnEnable()
    ...
    IEnumerator IUpdate()
    {
        while(true)
        {
            yield return YieldInstructionCache.RealTimeWaitForSeconds(0.2f);
            if (videoPlayer.isPlaying)
            {
                m_PlaybackProgress.fillAmount =
                    (float)(videoPlayer.length > 0 ? videoPlayer.time / videoPlayer.length : 0);
    ...
    public void SetProgressState(bool state)
    {
        m_PlaybackProgress.enabled = state;
    }

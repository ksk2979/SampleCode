...
public class SoundManager : Singleton<SoundManager>
{
    ...
    private AudioObject FindPlaySound(string audioID)
    {
        ...
        if (soundList.Count < 20)
        {
            ...
            if (audio == null) { return null; }
            if (GameManager.GetInstance._nowScene == EScene.E_GAME)
            {
                if (_cameraManager == null) { _cameraManager = CameraManager.GetInstance; }
                audio.GetTrans.SetParent(_cameraManager.GetTrans);
            }
            else
            {

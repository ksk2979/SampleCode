...
// 보스 유형의 자폭문어 (폭파)
public class EnemyExplosionType3Controller : EnemyController
{
    ...
    private void Update()
    {
        if (_attackSpawn)
        {
            ...
            if (_mobTime > 5f)
            {
                for (int i = 0; i < 3; ++i)
                {
                    if (GameManager.GetInstance.TestEditor) { SpawnTestManager.GetInstance.BossType2CreateEnemy(10002, _trans); }
                    else { if (_spawnPos != null) { _stageManager._chapter[_stageManager._cc].CreateForcedSpawnEnemy(10002, _spawnPos, _enemyStats, 0.2f); } else { _stageManager._chapter[_stageManager._cc].CreateForcedSpawnEnemy(10002, _trans, _enemyStats, 0.2f); } }
                }
                _mobTime = 0f;
            }
        }

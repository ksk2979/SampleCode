...
public class EnemyController : CharacterController
{
    ...
    private void Update()
    {
        ...
        float moveSpeed = _enemy.GetEnemyStats.Speed;
        ...
        RaycastHit2D frontPlayerHit = Physics2D.Raycast(
            _trans.position + new Vector3(-0.4f, 0.8f, 0f),
            Vector2.left,
            _checkDistance,
            _playerLayer
        );
        ...
        bool isBlocked = frontPlayerHit.collider != null || frontHit.collider != null;
        if (!isBlocked)
        {
            _trans.Translate(moveDir.normalized * (moveSpeed * Time.deltaTime), Space.World);
        }
    }

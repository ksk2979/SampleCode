...
public class AudioController : SingletonMonoBehaviour<AudioController>
{
    ...
    // 20160920 �ֹο� �߰�
    public bool IsPathMode = false;
    // 20170905 �ֹο� �߰�
    static bool _bgmOn = true;
    static public void SetBGMOn(bool bgmOn_)
    {
        _bgmOn = bgmOn_;
        UpdateBgmVolume();
    }
    static public bool GetBGMOn()
    {
        return _bgmOn;
    }

    static float _bgmVolume = 1f;
    static public void SetBGMVolume(float bgmVolume_)
    {
        _bgmVolume = bgmVolume_;
        UpdateBgmVolume();
    }
    static public float GetBGMVolume()
    {
        return _bgmVolume;
    }
    static void UpdateBgmVolume()
    {
        if (_bgmOn)
        {
            //SetCategoryVolume(csCommon.AUDIO_CATEGORY.bgm.ToString(), _bgmVolume);
        }
        else
        {
            //SetCategoryVolume(csCommon.AUDIO_CATEGORY.bgm.ToString(), 0f);
        }
    }
    //======================================
    ...
    {
#if TEST_ONLY_SKILLEFFECT
         if( audioID.Contains("skill_") )
             Debug.LogWarning("Play : " + audioID);
    ...
    static public AudioItem AddToCategory( AudioCategory category, AudioClip audioClip, string audioID )
    {
        ...
        audioSubItem.SetClip(audioClip);
    ...
    public AudioObject PlayAudioSubItem( AudioSubItem subItem, float volume, Vector3 worldPosition, Transform parentObj, float delay, float startTime, bool playWithoutAudioObject, AudioObject useExistingAudioObj, double dspTime = 0, bool playAsMusic = false )
    {
        ...
        {
            // 2016.02.29 �ֹο� ����
            audioPrefab.GetComponent<AudioSource>().PlayOneShot(
                //subItem.Clip,
                subItem.GetClip(),
                AudioObject.TransformVolume( volumeWithCategory ) ); // unfortunately produces warning message, but works (tested only with Unity 3.5)
        ...
        sndObj.primaryAudioSource.clip = subItem.GetClip();
    ...
    // 2016.02.29 �ֹο� �߰�
#if UNITY_EDITOR
    public void MakeToPathMode()
    {
        IsPathMode = true;
        for (int c = 0; c < AudioCategories.Length; c++)
        {
            AudioCategory audioCategory = AudioCategories[c];
            for (int i = 0; i < audioCategory.AudioItems.Length; i++)
            {
                AudioItem audioItem = audioCategory.AudioItems[i];
                for (int s = 0; s < audioItem.subItems.Length; s++)
                {
                    AudioSubItem audioSubItem = audioItem.subItems[s];
                    if (audioSubItem.Clip == null)
                    {
                        continue;
                    }
                    const string res = "/Resources/";
                    audioSubItem.Path = UnityEditor.AssetDatabase.GetAssetPath(audioSubItem.Clip);
                    int resIndex = audioSubItem.Path.IndexOf(res);
                    audioSubItem.Path = audioSubItem.Path.Remove(0, resIndex + res.Length);
                    audioSubItem.Path = System.IO.Path.ChangeExtension(audioSubItem.Path, null);
                    audioSubItem.Clip = null;
                    //Debug.Log(audioSubItem.Path);
                }
            }
        }
    }
